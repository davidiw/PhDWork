...
Examples of large-scale P2P systems include Skype$^\registered$, BitTorrent$^\registered$, and Gnutella.
Skype$^\registered$ is a voice over P2P system, whereas BitTorrent$^\registered$ and Gnutella are used for
...
providers, such as LogMeIn's Hamachi$^\registered$, a P2P VPN (virtual private network), may
...
public overlays with high availability, such as Skype$^\registered$, Gnutella, XMPP
(Extensible Messaging and Presence Protocol), and BitTorrent$^\registered$; by leveraging
...
Facebook$^\registered$ Chat.  XMPP also supports an overlay amongst servers forming through
...
a meta data file, such as in BitTorrent$^\registered$.  Drawbacks to this approach for small,
...
``P2P VPN's''~\cite{p2pvpn} use of a BitTorrent$^\registered$ tracker is similar to the host
...
with each BitTorrent$^\registered$ tracker individually and maintain a connection with each
of them, in order to handle cases where BitTorrent$^\registered$ trackers go offline.  In
addition, this does not use the BitTorrent$^\registered$ trackers in a normal fashion, so it
...
Alternatively, in a system like BitTorrent$^\registered$, peers could use the same tracker
...
10s to 100s of nodes.  For example, Amazon$^\registered$'s shopping cart runs on
Dynamo~\cite{dynamo} using a ``couple of hundred of nodes'' or less.  Facebook$^\registered$
...
Recently, Facebook$^\registered$ announced that they would be supporting XMPP as a means to
connect to Facebook$^\registered$ chat.  This was rather exciting and further motivated this
work, as Facebook$^\registered$ has over 400 million active users, which would have made
their XMPP overlay, potentially, the largest free-to-join overlay.
Unfortunately, Facebook$^\registered$ does not employ a traditional XMPP setup, instead it
...
BitTorrent$^\registered$ &
Default BitTorrent$^\registered$ implementations rely on a centralized tracker to provide the
...
of monitoring the state of the file distribution.  BitTorrent$^\registered$ specifies a
...
versions may potentially use UDP NAT traversal.  At which point, BitTorrent$^\registered$ may
...
\\ 
Skype$^\registered$ &
Skype$^\registered$ is a large-scale unstructured overlay, consisting of over a million
active peers, and primarily used for voice over P2P communication.  Skype$^\registered$, like
Gnutella, also has super peers, though the owners of Skype$^\registered$ provide
authentication and bootstrap servers.  Though Skype$^\registered$ is free-to-join, it
requires registration to use.
&
Skype$^\registered$ APIs provide no means for reflection.
&
Skype$^\registered$ supports applications, or add-ons, which can used to transparently
broadcast queries to a users friend to determine if the peer has the
application installed.  Thus Skype$^\registered$ does support rendezvous.
&
Skype$^\registered$ applications are allowed to route messages via the Skype$^\registered$ overlay, but
because Skype$^\registered$ lacks reflection, all communication must traverse the Skype$^\registered$
overlay.
\\ 
XMPP &
...
&
Similar to Skype$^\registered$, XMPP friends can broadcast queries to each other to find
...
There exists two popular Kademlia systems, one used by many BitTorrent$^\registered$ systems,
