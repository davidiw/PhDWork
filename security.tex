...
improve clarity.}.  Using a 100 ms delay like the simulator results in 9.2 and
9.3 seconds for the analytical model for private network sizes of 68 and 147,
respectively.

\begin{figure}[ht]
\centering
\includegraphics[width=4in]{figs/private.eps}
\caption{CDF of private overlay bootstrap time}
...
\multicolumn{1}{c}{Throughput (Mbit/s)} \\ \hline
Standard & .303 & 225.27 \\
...
security libraries and protocols.  In this example, the security filter
abstraction is used by senders and receivers through an EtE secured chat
application.  Each receiver and sender use the same abstracted model and thus
the chat application requires only high-level changes, such as verifying the
certificate used is Alice's and Bob's, to support security.
...
of peers.  To this date, I have implemented both a DTLS~\cite{dtls} (datagram
transport layer security) filter using the OpenSSL implementation of DTLS as
well as a protocol that reuses cryptographic libraries provided by .NET that
behaves similarly to IPsec.
...
Figure~\ref{fig:add_one}, which uses the following notations: secure (dtls),
insecure (nosec), PlanetLab (plab), and the Simulator (sim).

\begin{figure}[ht]
\centering
\includegraphics[width=4in]{figs/addone.eps}
\caption{A single node joining an insecure and secure overlay}
\label{fig:add_one}
\end{figure}
...
Figure~\ref{fig:bootstrap_eval}, which uses the following notations: secure
(dtls) and insecure (nosec).

\begin{figure}[ht]
\centering
\includegraphics[width=4in]{figs/bootstrap_time.eps}
\caption{Simulataneous bootstrapping of a secure and an insecure overlay}
...
Google$^\registered$ offers free hosting of Python web applications through Google$^\registered$ Apps, an
...
Trust or friendships already established in Facebook$^\registered$ used a now deprecated
technology that allowed desktop applications access to Facebook$^\registered$.  Certificate
exchange relied upon a web-based data store component provided by Facebook$^\registered$,
which was presented as a database.  When SocialVPN first contacts Facebook$^\registered$, it
...
Unfortunately, Facebook$^\registered$'s interface was poorly constructed and no longer
exists.  Each application had to embed in itself private key information to
authenticate itself with Facebook$^\registered$.  A malicious attacker could easily discover
this and change the stored data to suit their needs.  While Facebook$^\registered$ never gave
...
Facebook$^\registered$, the web application required hosting a third-party web site to
...
Unlike Facebook$^\registered$, XMPP is a well standardized, open system with many individual
...
provider.  After the unfortunate incident with Facebook$^\registered$, this open aspect was
...
Hamachi$^\registered$~\cite{hamachi} is a centralized P2P VPN provider using the web site for
authentication, peer discovery, and connection establishment.  While the
Hamachi$^\registered$ protocol claims to support various types of
...
central website.  The Hamachi$^\registered$ approach makes it easy for users to deploy their
...
obtain the certificate otherwise, it is decentralized.  In Hamachi$^\registered$, if the
central server goes offline, no new peers can join the VPN.

Campagnol VPN~\cite{campagnol} provides similar features to Hamachi$^\registered$: a P2P VPN
that relies on a central server for rendezvous or discovery of peers.  The key
differences between Hamachi$^\registered$ and Campagnol is that Campagnol is free and does
...
BitTorrent$^\registered$~\cite{bittorrent_security}, a P2P data sharing service,  supports
stream encryption between peers sharing files.  The purpose of BitTorrent$^\registered$
security is to obfuscate packets to prevent traffic shaping due to packet
sniffing. Thus BitTorrent$^\registered$ security uses a weak stream cipher, RC4, and lacks
peer authentication as symmetric keys are exchanged through an unauthenticated
Diffie-Hellman process.

Skype$^\registered$~\cite{skype} provides decentralized audio and video communication to over
a million concurrent users.  While Skype$^\registered$ does not provide documentation
detailing the security of its system, researchers~\cite{skype_auth,
skype_overview} have discovered that Skype$^\registered$ supports both EtE and PtP security.
Though similar to Hamachi$^\registered$, Skype$^\registered$ uses a KDC and does not let users setup their
own systems.
